...
class MiniMapApp extends Application {
  ...
  static get defaultOptions() {
    ...
    options.template = "modules/MiniMap-revamp/template.html";
    options.popOut = true;
...
function resSqau() {
  ...
  minimap_data["posx"] =
    (canvas.stage.pivot.x - minimap_data.paddedX) / w_ratio;
  minimap_data["posy"] =
    (canvas.stage.pivot.y - minimap_data.paddedY) / h_ratio;

  $("#mydiv")[0].style = `overflow: hidden; border: 2px solid red; height: ${
    minimap_data.squaresizeh / canvas.stage.scale.x
  }px; left: ${minimap_data.posx}px; position: absolute; top: ${
    minimap_data.posy
  }px; width: ${minimap_data.squaresizew / canvas.stage.scale.x}px;`;

  for (usera of game.users) {
    let char = usera.data.character;
    if (char) {
      let toks = game.scenes.viewed.data.tokens
        .filter((w) => w.actorLink == true && w.actorId == char)
        .map((l) => ({
          x: l.x,
          y: l.y,
          _id: l._id,
      ...
      for (tok of toks) {
        ...
        try {
          minimap_data.minimap_actors[
            tok._id
          ].style = `border-radius: 50%; position: absolute; height: 8px; width: 8px; background: ${usera.color}; left: ${aposx}px; top: ${aposy}px`;
        } catch {}
        // wrapper.removeChild
      }
    }
